...
**Where and why an LLM was used:** Codex served as the LLM reviewer for the 40 selected queries. It interpreted query text, retrieved titles, supplied metadata and behavioral evidence; assigned mismatch categories; identified apparently off-topic behavioral positives; and wrote explanations and proposed next checks. This semantic review helped distinguish multilingual wording, information-need differences and missing clinical constraints that token overlap and retrieval metrics alone cannot explain. The LLM also assisted with implementation and report drafting. Its case judgments are saved in `data/processed/phase10_review_notes.csv` as exploratory annotations, not clinician-validated labels.
